...
// %token  MAIN INT NUM VAR FLOAT CHAR PLUS MINUS MULT DIV MOD INC DEC ASSIGN GT LT EE GTE LTE NTE FBS FBC SBS SBC COMA SM PRINT IF ELSE FOR COLON SWITCH CASE DEFAULT 
...
ID1  : ID1 COMA VAR	 {       
     ...
     |VAR	         {             
							...
							if(check[$1.yylval.number] == 1)
							{	
								printf("error: multiple declaration for '%c' !!!\n",$1+'a');
							}
							else
							{
								check[$1.yylval.number] = 1;
							}
